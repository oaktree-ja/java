...
public class Exercise_TextFileCopy {
	...
	public static void main(String[]args) {
		...
		/*
		 * 파일을 복사하려고 해 
		 * C드라이브 밑 Windows 폴더 밑에 system.ini 파일을 복사하려고해
		 * main()메소드를 기준으로 생각했을 때 해당 파일을 읽어야 하기 때문에 입력 스트림을 사용해야 해
		 * 입력스트림을 사용해서 파일을 읽은 다음에 읽은 내용은 목적지 C:\\Temp\\system.txt 파일을 써야해
		 * main()메소드를 기준으로 생각 했을 때 해당 내용을 내보내야 하기 때문에 출력 스트림을 사용해야해
		 * 출력 스트림을 사용해서 읽은 내용은 system.txt파일에 저장돼
		 * 출력 스트림을 쓸 때에는 flush(),close()를 해주어야 해
